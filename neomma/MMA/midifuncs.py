...
def rawMidi(ln):
    ...
    if neomma.MMA.debug.debug:
        dPrint(
            "MIDI: Inserted raw midi in metatrack: %s"
...
def setChannelInit(ln):
        ...
        if o.upper() == "CHANNELS":
            for c in v.split(","):
                if "-" in c:
                    ...
                    if c1 <= 0 or c2 > 16:
                        error(
                            "ChannelInit: Valid channels range 1..16, not {}-{}.".format(
                                c1, c2
                            )
                        )
    ...
    if neomma.MMA.debug.debug:
        dPrint(
            "ChannelInit: '%s' queued to channels %s"
...
def trackWheel(name, ln):
    ...
    if ln:
        if len(ln) == 1 and ln[0] == "RESET":
            ...
            error(
                "{} MidiWheel: Unrecognized command(s) '{}'.".format(name, " ".join(ln))
            )

    for o, v in opts:
        if o == "OFFSET":
            startOffset = (
                stotick(v, "B", "MidiWheel Offset: '%s' is not recognized." % v)
                + gbl.tickOffset
            )
            if startOffset < 0:
                error(
                    "%s MidiWheel Offset: current pointer is '%s ticks' so start "
                    "'%s ticks' is before song start."
                    % (name, gbl.tickOffset, startOffset)
                )

        # No idea why 'Count' is valid?
        elif o == "DURATION" or o == "COUNT":
            duration = stotick(
                v, "B", "MidiWheel Duration: '%s' is not recognized." % v
            )
            if duration <= 0:
                error(
                    "%s MidiWheel Duration: Must be > 0, not '%s Ticks'."
                    % (name, duration)
                )
    ...
    if setOnly is not None:
        ...
        if neomma.MMA.debug.debug:
            dPrint(
                "MidiWheel %s: detuned to %s at %s ticks."
    ...
    if startValue < 0 or startValue > 16383:
        error(
            "{} MidiWheel: Start must be 0 ... 16383, not '{}'.".format(
                name, startValue
            )
        )
    ...
    if neomma.MMA.debug.debug:
        if not reset:
            ...
            rset = ""
        dPrint(
            "MidiWheel %s: detuned %s to %s from %s to %s. %s"
...
def trackPan(name, ln):
    ...
    def getv(v):
        try:
            ...
            return stoi(
                v, "Expecting integer value 0..127 or mnemonic (Left*, Center, Right*)."
            )
    ...
    if len(ln) not in (1, 3):
        error(
            "MidiPan %s: needs 1 arg [Value] OR 3 [Initvalue DestValue Beats/Measures]."
            % name
        )

    if len(ln) == 3:
        beats = (
            stotick(ln[2], "B", "MidiPan Duration: '%s' is not recognized." % ln[2])
            // gbl.BperQ
        )
        if beats < 1:
            error(
                "MidiPan %s: Beat/Measure value must be positive count, "
                "not '%s'." % (name, beats)
            )
    ...
    if neomma.MMA.debug.debug:
        if beats:
            dPrint(
                "Set %s MIDIPan from %s to %s over %s beats."
