...
def parseChordLine(l):
    ...
    for ll in l:
        if "@" in ll:  # we have something like "Cm@3.2"
            ...
            if beat >= gbl.QperBar + 1:
                error(
                    "Beat after @ must be less than {}, not '{}'.".format(
                        gbl.QperBar + 1, beat
                    )
                )
            ...
            if p > len(chordTabs) - 1:
                error(
                    "Too many chords specified in line. Max is %s. "
        ...
        if ctable:
            ...
            if beat != 0:
                error(
                    "The first chord must be at beat 1, not %s."
                    % ((beat / quarter) + 1)
                )
        ...
        if "z" in ch:
            if ch.count("z") > 1:
                error(
                    "Only one 'z' is permitted in the mute shortcut. '%s' is "
                    "illegal." % ch
                )
            ...
            if not c:  # no chord specified
                ...
                if r == "!":  # 'z!' is fine, mute all
                    ...
                    if lastChord:
                        ...
                        error(
                            "Unable to find a chord for '%s'. Try specifing the "
                            "chord name." % ch
                        )
                        if not c:
                            error(
                                "To use this shortcut to mute individual tracks "
                ...
                if r == "!":
                    error(
                        "'%s' is illegal. 'z!' mutes all tracks "
                        "so you can't include the chord." % ch
                    )
                elif not r:
                    error(
                        "'%s' is illegal. This notation is a shortcut,  You must specify tracks "
            ...
            if "-" in r:
                if r.count("-") > 1 or r.count("+") or r[0] != "-":
                    error(
                        "Track mute: Use of a '-' must be at start of line and "
            ...
            if "+" in r:
                ...
                for a in r[1:]:
                    if a not in muteTrackNames:
                        error(
                            "Track mute: track name '%s' not in permitted list '%s'."
                            % (a, muteTrackNames)
                        )
                r = r[1:]
                if "C" not in r:
    ...
    if ctable[-1].chStart >= endTime:
        error(
            "Maximum offset for chord '%s' must be less than %s, not '%s'."
