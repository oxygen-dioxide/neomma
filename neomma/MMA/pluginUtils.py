    ...
    for i, value in enumerate(s.split(",")):
        ...
        if "=" in value:
            ...
            if n not in res:
                if not allowUnknown:
                    error(
                        "Plugin {}: unexpected argument name {}.".format(_P().NAME, n)
                    )
                else:
                    n = n.upper()
            ...
            if i >= len(_P().ARGUMENTS):
                error(
                    "Plugin {}: unexpected argument provided ({}).".format(
                        _P().NAME, value
                    )
                )
            ...
            error(
                "Plugin {}: positional argument after named argument is not allowed.".format(
                    _P().NAME
                )
            )
...
def _printUsage(plugin):
    ...
    if plugin.TRACKTYPES is not None and len(plugin.TRACKTYPES) > 0:
        if len(plugin.TRACKTYPES) == 1:
            ...
            lines.append(
                (
                    4,
                    "Track type must be one of {}.".format(
                        ", ".join(plugin.TRACKTYPES[:-1])
                        + " or "
                        + plugin.TRACKTYPES[-1]
                    ),
                )
    ...
    if len(plugin.ARGUMENTS) > 0:
        ...
        lines.append(
            (4, "You can also use name=value syntax, like Call (see MMA documents).")
        )
        lines.append((0, ""))
        ...
        if line.strip() == "":
            ...
            for i, lline in enumerate(llines):
                if i == 0 and firstindent != indent:
                    print(
                        " " * firstindent + lline + (" \\" if len(llines) > 1 else "")
                    )
                else:
                    print(" " * indent + lline)
...
def checkTrackType(name):
    ...
    if len(_P().TRACKTYPES) == 1:
        ...
        error(
            "Plugin {}: track type must be one of {}.".format(
                _P().NAME,
                ", ".join(_P().TRACKTYPES[:-1]) + " or " + _P().TRACKTYPES[-1],
            )
        )
