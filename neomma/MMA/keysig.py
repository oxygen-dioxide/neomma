        ...
        if kname[0] in "ABCDEFG":
            ...
            if mi and kname in minKy:
                ...
                if mi:
                    ...
                    t = "major"
                error(
                    "KeySigs: keysignature '{} {}' is unknown/impossible.".format(
                        kname, t
                    )
                )
...
def getTranspose(ln, err):
    ...
    if len(ln) == 1:
        value = stoi(
            ln[0], "{}: Argument must be an integer, not '{}'".format(err, ln[0])
        )
    ...
    # Note: This does NOT properly set key name spellings (ie: C# vrs Db)
        ...
        if dir == "UP":
            ...
            error(
                "{}: Interval transpose needs 'UP/DOWN' as first arg, not '{}'.".format(
                    err, dir
                )
            )
        ...
        interval = "{} {}".format(
            getAbrev(ln[1], {a.split()[0] for a in ttable.keys()}),
        ...
        if interval in ttable:
            ...
            error(
                "%s: Unknown/invalid interval '%s'. Valid keys are: %s."
        ...
        error(
            "%s: Use single value for semitones (-3, 4) or proper "
...
def transpose(ln):
    ...
    if len(ln) and ln[0].upper() == "ADD":
        ...
        if gbl.transpose < -12 or gbl.transpose > 12:
            error(
                "Transose: Add option results in setting of '%s', must be -12 to 12."
                % gbl.transpose
            )
    else:
        gbl.transpose = getTranspose(ln, "Transpose")
