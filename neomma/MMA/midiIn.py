...
def midiinc(ln: list[str]):
    ...
    for cmd, opt in opts:
        ...
        if cmd == "FILE":
            ...
            insertOffset = stotick(
                opt, "B", "MidiInc Offset: '%s' is not recognized." % opt
            )
            ...
            if opt in ("OFF", "FALSE", "0"):
                ...
                stripSilence = stoi(
                    opt,
                    "MIdiInc StripSilence= expecting "
        ...
        # a trackname. Keep this as the last test!
            ...
            if ch is None:
                error(
                    "MidiInc: No MIDI channel specified for import of track %s." % cmd
                )
                return
            if ch < 1 or ch > 16:
    ...
    if mf.beatDivision != gbl.BperQ:
        warning(
            "MIDI file '%s' tick/beat of %s differs from MMA's "
            "%s. Will try to compensate" % (filename, mf.beatDivision, gbl.BperQ)
        )
    ...
    if report or verbose:  # try to be helpful
        ...
        for ch in sorted(mf.events.keys()):
            ...
            if (
                verbose and not report
            ):  # in verbose mode only list info for tracks we're using
                doit = 0
                for z in channels:
                    if z[1] == ch:
                        doit = 1
    ...
    if neomma.MMA.debug.debug:
        dPrint(
            "MidiInc: File=%s, Volume=%s, Octave=%s, Transpose=%s, Lyric=%s, "
    ...
    if stretch:
        if verbose:
            print(
                "Applying stretch to all events. Deltas will be multiplied by %s"
                % stretch
            )
    ...
    if stripSilence == False:
        if verbose:
            print(
                "Firstnote offset was %s. Being reset to start of file by StripSilence=Off."
                % mf.firstNote
            )
        mf.firstNote = 0
    ...
    if doText:
        ...
        if neomma.MMA.debug.debug:
            dPrint(
                "MidiInc text events: {} inserted, {} out of range.".format(inst, disc)
            )
    ...
    if doLyric:
        ...
        if neomma.MMA.debug.debug:
            dPrint(
                "MidiInc lyric events: {} inserted, {} out of range.".format(inst, disc)
            )

    for n, c, riffmode, printriff in channels:
        if not len(mf.events[c]):
            warning(
                "No data to assign from imported channel {} to track {}".format(
                    c + 1, n
                )
            )
    ...
    for tr, ch, riffmode, printriff in channels:
        ...
        if gbl.tnames[tr].disable:  # skip if disabled track
            if verbose:
                print(
                    "Skipping import of channel %s since track %s is disabled."
                    % (ch, tr)
                )
            continue
        ...
        if riffmode:
            ...
            if t.vtype not in ("MELODY", "SOLO"):
                error(
                    "MidiInc: Riff only works on Melody/Solo tracks, not '%s'." % t.name
                )

        t.clearPending()
        if t.voice[0] != t.ssvoice:
            gbl.mtrks[t.channel].addProgChange(
                gbl.tickOffset + insertOffset, t.voice[0], t.ssvoice
            )
        ...
        if verbose or 1:
            print(
                "Parsing imported file. Channel=%s Track=%s MIDI Channel=%s"
                % (ch, tr, channel)
            )
            if len(mf.events[ch]):
                print(
                    " Total events: %s; Event range: %sT %sT; Start/End Range: %sT %sT"
                    % (
                        len(mf.events[ch]),
                        mf.events[ch][0][0],
                        mf.events[ch][-1][0],
        ...
        if onNotes:
            ...
            warning(
                "MidiINC: Stuck MIDI note(s) '%s' turned off in %s."
        ...
        if riffmode:
            ...
            for a in sorted(evlist):
                if printriff and riffmode == 1:
                    ...
                    riffmode == 2
                ):  # sequence mode, create sequence line and push into input
...
def createRiff(riff, tname, riffTranspose):
    ...
    if missed:
        warning(
            "MidiInc Riff: conversion missed {} notes in track {}".format(missed, tname)
        )
