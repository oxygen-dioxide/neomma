...
def getOrnOpts(self):
    ...
    return (
        "Type=%s Chromatic=%s Duration=%s Count=%s Pad=%s,%s Offset=%s Volume=%s "
...
def setOrnament(self, ln):
    ...
    if (notopt and len(notopt) == 1 and notopt[0].upper() in ("NONE", "OFF")) or (
        not notopt and not optpair
    ):
        o["type"] = None
    ...
    if notopt or not optpair:
        error(
            "%s Ornament: expecting cmd=opt pairs,"
    ...
    for cmd, opt in optpair:
        ...
        if cmd == "TYPE":
            if opt in (
                ...
                error(
                    "{} Ornament Type: '{}' is an unknown type.".format(self.name, opt)
                )
            ...
            if o["count"] <= 0:
                error(
                    "%s Ornament Count: value must be positive integer, not '%s'."
                    % (self.name, opt)
                )
            ...
            if v < 0 or v > 100:
                error(
                    "%s Ornament Duration: values must be greater "
            ...
            for v in opt.split(",", 1):
                ...
                if v < -100 or v > 100:
                    error(
                        "{} Ornament Pad: values must be -100...100, not '{}'.".format(
                            self.name, v
                        )
                    )
                o["pad"].append(v / 100)
            ...
            if opt not in ("ABOVE", "BELOW", "RANDOM"):
                error(
                    "%s Ornament Place: must be ABOVE, BELOW or RANDOM, not '%s'."
                    % (self.name, opt)
                )
            o["place"] = opt
            ...
            if t > 0 and t <= 1000:
                ...
                error(
                    "%s Ornament Volume: must be greater "
            ...
            if v < 0 or v > 100:
                error(
                    "{} Ornament Rskip: must be 0 .. 100, not '{}'.".format(
                        self.name, v
                    )
                )
            o["rskip"] = v / 100
            ...
            n1, n2 = neomma.MMA.pat.getRndPair(
                opt, "%s Ornament" % self.name, -100, 100
            )
            o["rvolume"] = (n1 / 100.0, n2 / 100.0)
            ...
            if opt != "ALL":
                for t in opt.split(","):
                    ...
                    if v < 1 or v > gbl.seqSize:
                        warning(
                            "%s Ornament Bars: setting of %s may be "
...
def getNOdur(o, noteDur, ornamentDur):
    ...
    return int(noteDur + (noteDur * o["pad"][0])), int(
        ornamentDur + (ornamentDur * o["pad"][1])
    )
...
def doOrnament(self, nlist, scale, p):
    ...
    if (
        (orn_beats and p.offset not in orn_beats)
    ...
    # start of the different option code.
        ...
        for t in count:
            ...
            self.sendChord(
                [[tnote, getOvolume(o, nvol)]], noteDur, offset + odur + orn_offset
            )
            offset += odur + odur
        retoffset = p.offset
        ...
        self.sendChord(
            [[note, getOvolume(o, nvol)]], ornamentDur, p.offset + orn_offset
        )
        self.sendChord(
            [[onote, getOvolume(o, nvol)]], ornamentDur, p.offset + odur + orn_offset
        )
        ...
        noteDur, ornamentDur = getNOdur(o, ndur, odur)
        self.sendChord(
            [[note1, getOvolume(o, nvol)]], ornamentDur, p.offset + orn_offset
        )
        self.sendChord(
            [[note, getOvolume(o, nvol)]], ornamentDur, p.offset + odur + orn_offset
        )
        self.sendChord(
            [[note2, getOvolume(o, nvol)]],
            ornamentDur,
            p.offset + (odur * 2) + orn_offset,
        )
        ...
        self.sendChord(
            [(note2, getOvolume(o, nvol))], ornamentDur, p.offset + ndur + orn_offset
        )
        self.sendChord(
            [(note1, getOvolume(o, nvol))],
            ornamentDur,
            p.offset + ndur + odur + orn_offset,
        )
        self.sendChord(
            [(note2, getOvolume(o, nvol))],
            ornamentDur,
            p.offset + ndur + odur + odur + orn_offset,
        )
        retoffset = p.offset
