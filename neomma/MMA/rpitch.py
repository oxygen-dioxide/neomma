...
class Rpitch:
    def __init__(self):
        ...
        self.rate = 0.25  # 25%, apply to all notes
...
def getOpts(self):
    ...
    return "Rate={} ScaleType={} Offsets={} Bars={} Beats={}".format(
        rate, scale, offsets, bars, beats
    )
        ...
        for o, c in opts:
            if o == "SCALETYPE" or o == "SCALE":
                if not c in ("CHROMATIC", "SCALE", "CHORD"):
                    error(
                        "%s Rpitch ScaleType: Only Chromatic, Scale and Chord "
                        "are valid." % self.name
                    )
                rp.scale = c
                ...
                if c:
                    for a in c.split(","):
                        ...
                        if a < 0:
                            error(
                                "{} Bars must be positive values, not '{}'.".format(
                                    msg, a
                                )
                            )
                        if a < 1 or a > gbl.seqSize:
                            warning(
                                "%s Bars: setting of %s may be ignored since "
                ...
                for t in c.split(","):
                    ...
                    if v < 1 or v > gbl.QperBar:
                        error(
                            "%s Beat values must be in range 1 to %s, not '%s'."
                            % (msg, gbl.QperBar, v)
                        )
