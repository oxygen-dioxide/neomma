...
class Chord(PC):
    ...
    def setVoicing(self, ln):
        ...
        for mode, val in ln:
            if mode == "MODE":
                ...
                if val < 1 or val > 30:
                    error(
                        "Voicing Range: Arg out-of-range; must be 1 to 30, not '%s'."
                        % val
                    )
                ...
                val = stoi(
                    val, "Argument for %s VOICING CENTER must be a value." % self.name
                )

                if val < 1 or val > 12:
                    error(
                        "VOICING CENTER: arg out-of-range; must be 1 to 12, not '%s'."
                        % val
                    )
                ...
                val = stoi(
                    val, "Argument for %s VOICING RANDOM must be a value" % self.name
                )
                ...
                val = stoi(
                    val, "Argument for %s VOICING MOVE  must be a value" % self.name
                )
                ...
                val = stoi(
                    val, "Argument for %s VOICING DIR must be a value" % self.name
                )
        ...
        if neomma.MMA.debug.debug:
            v = self.voicing
            dPrint(
                "Set %s Voicing MODE=%s RANGE=%s CENTER=%s RMOVE=%s MOVE=%s DIR=%s "
    ...
    def setDupRoot(self, ln):
        ...
        for n in ln:
            ...
            for v in n.split(","):
                v = stoi(
                    v,
                    "%s DupRoot: Argument must be a value, not '%s'." % (self.name, v),
                )

                if v < -9 or v > 9:
                    error(
                        "{} DupRoot: '{}' out-of-range; must be -9 to 9.".format(
                            self.name, v
                        )
                    )
    ...
    def getPgroup(self, ev):
        ...
        if len(ev) < 3:
            error(
                "There must be at least 3 items in each group "
                ...
                while 1:
                    ...
                    if lo[1]:
                        offset = neomma.MMA.ornament.doOrnament(
                            self,
                            [lo],
                            self.getChordInPos(p.offset, ctable).chord.scaleList,
                            p,
                        )
                        break
                    elif not loo:
