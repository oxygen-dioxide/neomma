...
def libUpdate():
    ...
    for lib in neomma.MMA.paths.libDirs:
        ...
        # it,it's probably due to a permission error. So, just dump out.
            ...
            if os.path.exists(file):
                try:
                    ...
                    error(
                        "Unable to delete existing database file '%s'. Are you root?"
                        % file
                    )
        ...
        outpath.write(b"### mmaDB ... AUTOGENERATED BINARY DATA. DO NOT EDIT!!!\n")
            ...
            for kk in dbKeys[idx:]:
                for g in db[k]:
                    if g in db[kk]:
                        dupMessage.append(
                            "   Lib %s: %s & %s\n"
                            % (lib, os.path.basename(k), os.path.basename(kk))
                        )
                        break
            idx += 1
...
def loadDB(d):
    ...
    try:
        ...
        error(
            "Incompatible database found. Maybe a result"
    ...
    for a in set(dirs):
        if not os.path.exists(a):
            warning(
                "There appears to be an error in the database.\n"
...
def findGroove(targ):
    ...
    if ":" in targ:
        ...
        # lib the first one found will be used.
            ...
            if not fpath:
                error(
                    "Can't locate library file: %s! Groove: %s can't be found either."
                    % (dirfile, targ)
                )
