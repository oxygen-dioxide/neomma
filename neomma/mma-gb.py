...
def update_groove_db(root, dir, textbox=None):
    ...
    for f in files:
        ...
        if os.path.isdir(path):
            ...
            try:
                pp = subprocess.Popen(
                    [MMA, "-Dbo", path], stdout=subprocess.PIPE, shell=sub_shell
                )
                output = pp.communicate()[0]
            except:
                msg = (
                    "Error in reading mma file. Is MMA current?\n"
...
def write_db(root, dbName, db, textbox=None):
    ...
    try:
        ...
        msg = "Error creating groove database file '%s'. Do you need to be root?" % path
...
def dohelp(hw=[None]):
    ...
    for l in [
        "Simple browser to view MMA grooves.",
        "",
        "Key bindings:",
...
class Application:
    def __init__(self):
        ...
        self.e_tempo = makeEntry(
            self.f1, label="Tempo", text=opt_tempo, row=0, column=0
        )
        self.e_keysig = makeEntry(
            self.f1, label="Key Signature", text=opt_keysig, row=0, column=1
        )
        self.f1.grid(column=0, row=1, sticky=W)
        self.e_chords = makeEntry(
            self.f2, label="Chords", text=opt_chords, row=0, column=0
        )
        self.e_count = makeEntry(
            self.f2, label="Count", text=opt_count, row=0, column=1
        )
    ...
    def displayFile(self, fileName, win=[None, None]):
        ...
        try:
            ...
            b.insert(
                END, "Can't access file %s. This is configuration error." % fileName
            )
        else:
            for l in infile:
    ...
    def launchSubprocess(self, processTitle, processCommand, fileName):
        try:
            p = subprocess.Popen(
                [processCommand, fileName], stderr=subprocess.PIPE, shell=sub_shell
            )
            output = p.communicate()[0]
        except:
            tkMessageBox.showerror(
