...
pu.addArgument(
    "Volume",
    "80",
    "volume for strums, can be specified for each string (see Plectrum docs)",
)
pu.addArgument(
    "VolumeMuted",
    "60",
    "volume for mute, can be specified for each string (see Plectrum docs)",
)
pu.addArgument(
    "VolumeEmph",
    "100",
    "volume for emphatized strums, can be specified for each string (see Plectrum docs)",
)
    ...
    for bar_pattern in pattern.split(";"):
        ...
        for i, c in enumerate(bar_pattern):
            ...
            if c == "u":
                ...
                # printUsage()
                pu.error(
                    "{}: unrecognized command in strum pattern: '{}'.".format(
                        pu.getName(), c
                    )
                )

        bar_normal = (
            ("{" + "; ".join(strums_normal) + ";}") if len(strums_normal) > 0 else "z"
        )
        bar_muted = (
            ("{" + "; ".join(strums_muted) + ";}") if len(strums_muted) > 0 else "z"
        )
